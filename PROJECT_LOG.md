...
## [Thurs 08.01.25]
1. Refactor carousel
    - Navigation and state no longer mixed
    - Removed scroll-based index math and eliminated the snap-back loop by making activateIndex UI-only & moving all scrolling into navigateToIndex;
    - Using `IntersectionObserver` for single source of truth for which slide is visible
    - Refactor click and keydown handlers to call navigation instead of mutate state
2. CSS
    - Remove empty `components.css`
    - Make SVGs click-through to not impede theme toggle activation
MISC.
    - Add TypeScript, script, & tsconfig.json

## [Wed 07.01.25]
1. Begin tidying up JS
2. Added favicons
3. CSS
    - Add `@supports` + base fallback color for gradient text
